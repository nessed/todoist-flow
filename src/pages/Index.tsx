...
export default function Index() {
  ...
  const consistencyScore = useMemo(() => {
    ...
    const normalized = Math.min(streakDays, 21) / 21;
    return Math.round(normalized * 100);
  }, [streakDays]);

  const consistencyMessage = useMemo(() => {
    if (streakDays <= 0) {
      return "Kickstart a new streak to unlock deeper momentum insights.";
    }

    if (streakDays < 5) {
      return "Momentum is building — keep the cadence for a breakthrough streak.";
    }

    if (streakDays < 14) {
      return "Great rhythm — DoneGlow is tracking a strong streak.";
    }
  ...
  const totalDaysInRange = useMemo(() => {
    ...
    const difference = differenceInCalendarDays(end, start) + 1;
  ...
  const averagePerDayLabel = useMemo(() => {
    ...
    return `${averagePerDay < 0.1 ? averagePerDay.toFixed(2) : averagePerDay.toFixed(1)} / day`;
  }, [averagePerDay]);
  ...
  const rangeDescriptor = useMemo(() => {
    ...
    return `${totalDaysInRange} ${totalDaysInRange === 1 ? "day" : "days"} in view`;
  }, [totalDaysInRange]);
  ...
  const heroSubheading = useMemo(() => {
    if (!filteredTasks.length) {
      ...
      return userFirstName ? `${userFirstName}, ${message}` : message;
    }

    const completions = numberFormatter.format(filteredTasks.length);
    const prefix = userFirstName ? `${userFirstName}, ` : "";
    return `${prefix}${completions} tasks completed ${formattedRangeLabel}. Keep the rhythm with ${averagePerDayLabel} and today's action plan.`;
  }, [
    averagePerDayLabel,
    filteredTasks.length,
    formattedRangeLabel,
              ...
              <div className="relative flex flex-col gap-10">
                <div className="space-y-6">
                  <div className="flex flex-wrap items-center gap-3">
                    <Badge
                      variant="secondary"
                      className="w-fit rounded-full border border-primary/30 bg-primary/15 px-4 py-1 text-[11px] uppercase tracking-[0.32em] text-primary"
                    >
                      Daily briefing
                    </Badge>
                    <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-background/70 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.32em] text-muted-foreground/70">
                      {rangeDescriptor}
                    </span>
                    {!useSampleData && hasConnectedAccount ? (
                      <span className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-primary/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.32em] text-primary">
                        <Sparkles className="h-3 w-3" />
                        Live Todoist sync
                      </span>
                    ) : null}
                    {useSampleData ? (
                      <span className="inline-flex items-center gap-2 rounded-full border border-primary/40 bg-primary/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.32em] text-primary">
                        Sample mode
                      </span>
                    ) : null}
                  </div>
                  <div className="space-y-4">
                    <h2 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">{heroHeadline}</h2>
                    <p className="max-w-3xl text-sm text-muted-foreground/80 sm:text-base">{heroSubheading}</p>
                  </div>
                </div>
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-background/80 p-5 shadow-[0_24px_64px_-40px_rgba(15,23,42,0.6)]">
                    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(140deg,hsla(var(--primary),0.18),transparent_75%)]" />
                    <div className="relative flex gap-4">
                      <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-primary/20 text-primary">
                        <Target className="h-6 w-6" />
                      </div>
                      <div className="space-y-2">
                        <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Focus cue</p>
                        <p className="text-sm text-muted-foreground/80">{focusTip}</p>
                      </div>
                    </div>
                  </div>
                  <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-background/80 p-5 shadow-[0_24px_64px_-40px_rgba(15,23,42,0.6)]">
                    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(160deg,hsla(var(--secondary),0.16),transparent_70%)]" />
                    <div className="relative space-y-3">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Range overview</p>
                        <Badge
                          variant="outline"
                          className="rounded-full border-white/20 bg-background/70 px-3 py-1 text-[10px] uppercase tracking-[0.32em] text-muted-foreground/70"
                        >
                          {rangeDescriptor}
                        </Badge>
                      </div>
                      <p className="text-sm text-muted-foreground/80">
                        Average pace <span className="font-semibold text-foreground">{averagePerDayLabel}</span>
                      </p>
                      <div className="h-2 overflow-hidden rounded-full bg-white/10">
                        <div
                          className="h-full rounded-full bg-gradient-to-r from-primary via-primary/80 to-primary/60"
                          style={{ width: `${Math.min(100, Math.max(12, consistencyScore))}%` }}
                        />
                      </div>
                      <p className="text-xs text-muted-foreground/70">{consistencyMessage}</p>
                    </div>
                  </div>
                  <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-background/80 p-5 shadow-[0_24px_64px_-40px_rgba(15,23,42,0.6)] md:col-span-2 xl:col-span-1">
                    <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(155deg,hsla(var(--accent),0.18),transparent_70%)]" />
                    <div className="relative flex items-start gap-4">
                      <Avatar className="h-12 w-12 border border-white/10">
                        {resolvedAvatarUrl ? (
                          <AvatarImage src={resolvedAvatarUrl} alt={userProfile?.full_name ?? "DoneGlow explorer"} />
                        ) : null}
                        <AvatarFallback className="text-base font-semibold text-foreground">{userInitials}</AvatarFallback>
                      </Avatar>
                      <div className="space-y-2">
                        <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Connected as</p>
                        <p className="text-base font-semibold text-foreground">
                          {userProfile?.full_name ?? "DoneGlow Explorer"}
                        </p>
                        <p className="text-xs text-muted-foreground/70">
                          {userProfile?.timezone ? `Working in ${userProfile.timezone}` : "Connected Todoist account"}
                        </p>
                        <div className="flex flex-wrap items-center gap-2 pt-1 text-[11px] text-muted-foreground/70">
                          {userProfile?.email ? (
                            <span className="rounded-full border border-white/10 bg-background/70 px-2 py-0.5">
                              {userProfile.email}
                            </span>
                          ) : null}
                          {useSampleData ? (
                            <Badge
                              variant="outline"
                              className="rounded-full border border-primary/40 bg-primary/10 px-2 py-0.5 text-[10px] uppercase tracking-[0.28em] text-primary"
                            >
                              Sample mode
                            </Badge>
                          ) : null}
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                  {heroHighlights.map((highlight) => (
                    <div
                      key={highlight.label}
                      className="relative overflow-hidden rounded-3xl border border-white/10 bg-background/80 p-5 shadow-[0_18px_52px_-36px_rgba(15,23,42,0.55)]"
                    >
                      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(150deg,hsla(var(--foreground),0.06),transparent_68%)]" />
                      <div className="relative space-y-3">
                        <div className="flex items-center gap-3">
                          <div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${highlight.iconClass}`}>
                            <highlight.icon className="h-5 w-5" />
                          </div>
                          <span className="text-xs font-medium uppercase tracking-[0.28em] text-muted-foreground/70">
                            {highlight.label}
                          </span>
                        </div>
            ...
            <aside className="relative hidden overflow-hidden rounded-[2.5rem] border border-white/10 bg-background/80 p-8 shadow-[0_28px_60px_-40px_rgba(15,23,42,0.6)] xl:flex">
              ...
              <div className="relative flex h-full flex-col gap-6">
                ...
                <div className="space-y-5 text-sm text-muted-foreground">
                  <div>
                    <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Current streak</p>
                    <p className="mt-1 text-lg font-semibold text-foreground">{recapStats.currentStreak} days</p>
                    <p className="text-xs text-muted-foreground/70">{consistencyMessage}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Top project</p>
                ...
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-muted-foreground/70">Saved filters</p>
